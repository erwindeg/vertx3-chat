...
public class MainVerticle extends AbstractVerticle {
	...
	public void start() throws Exception {
		...
		vertx.eventBus().consumer(
				"history",
				m -> proxy.find("messages", new JsonObject(),
						res -> sendMessages(((JsonObject) m.body()).getString("channel"), res)));
		matcher.matchMethod(
				HttpMethod.GET,
				"/api/history",
				req -> proxy.find("messages", new JsonObject(),
						res -> req.response().end(new JsonArray(res.result()).toString())));
